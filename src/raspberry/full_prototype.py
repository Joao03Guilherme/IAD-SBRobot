...
from gyroscope_controller import MPU6050
from angle_accelarometer import AngleSensor
...
class SelfBalancingRobot:
    ...
    def __init__(self):
        ...
        # Create the angle sensor
        self.angle_sensor = AngleSensor(i2c)
        time.sleep(1)  # Let sensors stabilize
        
        # Create motor controller first
        self.motor_controller = MotorController(MOTOR_CONFIG)
        
        # Use the Driving class from drive.py
        self.driver = Driving(self.angle_sensor, self.motor_controller)
    ...
    def send_help_message(self):
        ...
        for cmd_num, cmd_info in COMMANDS.items():
            help_text += f"{cmd_num}: {cmd_info['action']} - {cmd_info['description']}\n"
    ...
    def show_status(self):
        ...
        status = {
            ...
            "speed": self.speed,
            "turn": self.turn,
            "angle": round(self.driver.angle_data[-1] if self.driver.angle_data else 0, 2),
            ...
            if action == "START":
                ...
                if len(parts) >= 2:  # Changed from >= 3 to >= 2
                    try:
                        ...
                        self.turn = int(parts[2]) if len(parts) > 2 else 0  # Make turn optional
    ...
    def send_telemetry(self):
        ...
        telemetry = f"A:{angle:.2f},S:{self.speed},T:{self.turn},R:{1 if self.running else 0}"
    ...
    def main_loop(self):
        ...
        print(f"Entering main loop with speed={self.speed}, turn={self.turn}, running={self.running}")
        ...
        try:
            while self.running:
                ...
                print(f"forward() result: angle={angle:.2f}, left={left}, right={right}")
