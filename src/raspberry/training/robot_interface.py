...
class RobotInterface:
    ...
    def __init__(self, connection_method="bluetooth", config_file=None):
        ...
        self.telemetry_buffer = deque(maxlen=1000)  # Limited buffer size to prevent memory issues
            ...
            if data.startswith("A:"):
                ...
                if 'A' in values and 'B' in values and 'S' in values:
                    ...
                    if hasattr(self, 'prev_speed') and hasattr(self, 'prev_time'):
                        ...
                        if dt > 0:
                            self.last_state['acceleration'] = (speed - self.prev_speed) / dt
                        else:
    ...
    def get_state(self):
        ...
        return [
            ...
            self.last_state.get('angle', 0.0) - self.last_state.get('target_angle', 0.0) / 30.0,  # Normalized error
    ...
    def update_parameters(self, parameters):
        ...
        for param, value in parameters.items():
            try:
                ...
                if safe_value != value:
                    print(f"Warning: Parameter '{param}' value {value} outside allowed range [{min_val}, {max_val}], clamped to {safe_value}")
    ...
    def _calculate_metrics(self, states, commands):
        ...
        metrics = {
            ...
            'speed_error': np.sqrt(np.mean([(target_speeds[i] - speeds[i])**2 for i in range(min(len(speeds), len(target_speeds)))])),
