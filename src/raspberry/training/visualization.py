...
class TrainingVisualizer:
    ...
    def start_real_time_visualization(self, update_interval=2.0):
        ...
        try:
            ...
            self.thread = threading.Thread(
                target=self._update_plots_thread, args=(update_interval,)
            )
    ...
    def update_plots(self):
        ...
        self._plot_parameters(
            self.axs[0], param_history, episode_markers, episode_numbers
        )

        # Plot reward history
        self._plot_rewards(
            self.axs[1], reward_history, episode_markers, episode_numbers
        )
    ...
    def _plot_scores(self, ax, score_history, episode_markers, episode_numbers):
        ...
        filtered_scores = [
            min(s, 1000) for s in score_history
        ...
        if len(filtered_scores) > window:
            moving_avg = np.convolve(
                filtered_scores, np.ones(window) / window, mode="valid"
            )
            ax.plot(
                range(window - 1, len(filtered_scores)),
                moving_avg,
                label=f"{window}-step Moving Avg",
                color="darkred",
                linestyle="--",
