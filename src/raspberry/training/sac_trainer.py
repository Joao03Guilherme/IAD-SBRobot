...
class SACTrainer:
    ...
    def normalize_action(self, action):
        ...
        for i, (param_name, (min_val, max_val)) in enumerate(self.param_ranges.items()):
            ...
            normalized[param_name] = min_val + (action[i] + 1) * 0.5 * (max_val - min_val)
        return normalized
    ...
    def denormalize_action(self, parameters):
        ...
        for i, (param_name, (min_val, max_val)) in enumerate(self.param_ranges.items()):
            if param_name in parameters:
                ...
                denormalized[i] = (parameters[param_name] - min_val) / (max_val - min_val) * 2 - 1
        return denormalized
    ...
    def train(self, num_episodes=100, max_steps=200, warm_up_steps=1000, update_freq=1):
        ...
        for episode in range(num_episodes):
            ...
            if self.use_visualization:
                self.visualizer.add_data_point(current_params, 0, float('inf'), new_episode=True)
            ...
            while not done and step < max_steps:
                ...
                if not update_success:
                    ...
                    if recovery_attempts < max_recovery_attempts:
                        print(f"Attempting recovery ({recovery_attempts+1}/{max_recovery_attempts})...")
                        recovery_attempts += 1
                        try:
                ...
                if self.use_visualization:
                    self.visualizer.add_data_point(current_params, reward, metrics['overall_score'])
                ...
                # Print progress
                print(f"  Step {step}: Reward={reward:.2f}, Score={metrics['overall_score']:.2f}")
                print(f"    Angle: mean={metrics['angle_mean']:.2f}°, max={metrics['angle_max']:.2f}°")
                print(f"    Speed error: {metrics['speed_error']:.2f}, Accel: {metrics['acceleration_mean']:.2f}")
