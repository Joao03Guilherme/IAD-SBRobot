...
class TrainingEnvironment:
        ...
        try:
            ...
            if not self.use_physical:
                return self.robot.forward(
                    target_speed=target_speed, turn_bias=turn_bias
                )
            ...
            if abs(current_angle) > self.safety_limits["emergency_stop_tilt"]:
                print(
                    f"EMERGENCY STOP: Tilt angle {current_angle:.2f}° exceeds safety limit"
                )
                self.robot.stop()
            ...
            if abs(current_angle) > self.safety_limits["max_tilt"] * 0.7:
                ...
                tilt_factor = (self.safety_limits["max_tilt"] - abs(current_angle)) / (
                    self.safety_limits["max_tilt"] * 0.3
                )
                safe_speed = target_speed * max(0, min(1, tilt_factor))

            # Apply absolute speed limit
            safe_speed = max(
                -self.safety_limits["max_speed"],
                min(self.safety_limits["max_speed"], safe_speed),
            )
    ...
    def get_state(self):
        ...
        try:
            ...
            recent_accels = (
                list(self.robot.acceleration_history)
                if hasattr(self.robot, "acceleration_history")
                else [0]
            )
            ...
            error_derivative = self.robot.last_error - (
                self.robot.angle_data[-2] if len(self.robot.angle_data) > 1 else 0
            )

            # Get recent motor powers
            left_power = (
                self.robot.left_power_data[-1] if self.robot.left_power_data else 0
            )
            right_power = (
                self.robot.right_power_data[-1] if self.robot.right_power_data else 0
            )
    ...
    def is_terminal_state(self):
        ...
        try:
            ...
            if (
                not self.use_physical
                and hasattr(self.robot, "fallen")
                and self.robot.fallen
            ):
                return True
...
class ParameterTrainer:
    ...
    def __init__(
        self, episodes=100, max_steps=1000, use_model=False, use_physical=False
    ):
    ...
    def _get_reward(self, state, prev_state=None):
        ...
        try:
            ...
            if prev_state is not None:
                ...
                if (
                    angle * prev_angle < 0 and abs(angle) > 0.05
                ):  # Only penalize significant oscillations
    ...
    def _select_action_random(self):
        ...
        if random.random() < self.exploration_rate:
            ...
            for param in self.parameter_ranges.keys():
                ...
                range_size = (
                    self.parameter_ranges[param][1] - self.parameter_ranges[param][0]
                )
    ...
    def _update_q_network(self):
        ...
        with torch.no_grad():
            ...
            target_q = (
                reward_batch + (1 - done_batch) * self.discount_factor * max_next_q
            )
    ...
    def train_episode(self):
        ...
        if self.results["episode_rewards"] and random.random() < 0.7:
            ...
            for param in self.current_parameters:
                ...
                variation_scale = max(
                    0.05 * (1.0 - len(self.results["episode_rewards"]) / self.episodes),
                    0.01,
                )
                self.current_parameters[param] += (
                    random.uniform(-variation_scale, variation_scale) * range_size
                )
        else:
        ...
        if (
            self.use_model
            and len(self.results["episode_rewards"]) % self.target_update_frequency == 0
        ):
            self.target_network.load_state_dict(self.q_network.state_dict())
    ...
    def train(self):
        ...
        for episode in range(self.episodes):
            ...
            # Print progress
            print(
                f"Episode {episode+1}/{self.episodes} - Steps: {steps} - Reward: {episode_reward:.2f}"
            )
            ...
            if (episode + 1) % 10 == 0:
                ...
                if self.avg_rewards:
                    print(
                        f"10-episode moving average reward: {self.avg_rewards[-1]:.2f}"
                    )
        ...
        self.results["training_metadata"]["end_time"] = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def load_results(self, filename="parameter_training_results.pkl"):
        ...
        try:
            if os.path.exists(filename):
                ...
                self.best_parameters = self.results.get(
                    "best_parameters", self._get_default_parameters()
                )
...
def test_robot_with_parameters(
    parameters, duration=10, test_patterns=True, use_physical=False
):
    ...
    print(
        f"Testing {'physical robot' if use_physical else 'simulation'} with parameters for {duration} seconds..."
    )
    print("Parameters:")
    for param, value in parameters.items():
        print(f"  {param}: {value:.4f}")
    ...
    try:
        while time.time() - start_time < duration:
            ...
            print(
                f"Target: {target:5.2f} | Angle: {angle:6.2f}° | Speed: {speed:5.2f} m/s | Accel: {accel:5.2f} m/s²"
            )
    ...
    if angles and speeds and targets:
        ...
        speed_error = math.sqrt(
            sum((targets[i] - speeds[i]) ** 2 for i in range(len(speeds))) / len(speeds)
        )
        ...
        print(
            f"Average acceleration: {avg_accel:.2f} m/s² (lower for smoother control)"
        )
...
def transfer_learning(sim_parameters, physical_episodes=10, max_steps=200):
    ...
    trainer = ParameterTrainer(
        episodes=physical_episodes, max_steps=max_steps, use_physical=True
    )
...
def main():
    ...
    if HAS_TORCH:
        ...
        use_model = (
            input("Use neural network for training? (y/n): ").lower().startswith("y")
        )
    else:
    ...
    use_physical = (
        input("Use physical robot for training? (y/n, default n): ")
        .lower()
        .startswith("y")
    )
    ...
    if trainer.load_results():
        ...
        choice = input(
            "\nDo you want to (c)ontinue training, (t)est parameters, (p)hysical transfer learning, or (q)uit? "
        ).lower()
        if choice.startswith("c"):
            ...
            test_patterns = (
                input("Test different motion patterns? (y/n, default y): ").lower()
                != "n"
            )
            use_phys = (
                input("Test on physical robot? (y/n, default n): ")
                .lower()
                .startswith("y")
            )
            test_robot_with_parameters(
                trainer.best_parameters, duration, test_patterns, use_phys
            )
        elif choice.startswith("p") and not use_physical:
            # Transfer learning from simulation to physical robot
            phys_episodes = int(
                input("Number of physical robot episodes (default 10): ") or "10"
            )
            phys_max_steps = int(
                input("Maximum steps per episode (default 200): ") or "200"
            )
            refined_params = transfer_learning(
                trainer.best_parameters, phys_episodes, phys_max_steps
            )
            # Test the refined parameters
            test_choice = input(
                "Test the refined parameters? (y/n, default y): "
            ).lower()
            if test_choice != "n":
        ...
        if not use_physical:
            transfer_choice = input(
                "Transfer learning to physical robot? (y/n, default n): "
            ).lower()
            if transfer_choice.startswith("y"):
                ...
                refined_params = transfer_learning(
                    best_parameters, phys_episodes, phys_max_steps
                )
                # Test the refined parameters
                test_choice = input(
                    "Test the refined parameters? (y/n, default y): "
