...
class RobotSimulation:
    ...
    def __init__(self, physical_params=None):
        ...
        self.state[2] = np.radians(1.0)  # Start with a small tilt to make balance necessary
    ...
    def init_sensor_data(self):
        ...
        self.angle_data_queue = deque(maxlen=100)  # Directly provide queue for parameter training
        ...
        self.speed_data_queue = deque(maxlen=100)  # Directly provide queue for parameter training
    ...
    def simulate(self, dt=0.01):
        ...
        left_power += np.random.normal(0, motor_noise * abs(left_power)) if left_power != 0 else 0
        right_power += np.random.normal(0, motor_noise * abs(right_power)) if right_power != 0 else 0
        ...
        max_torque = motor_torque_constant * self.physical_params["motor_voltage"] / self.physical_params["motor_resistance"] * motor_efficiency
        ...
        if np.random.random() < self.physical_params["disturbance_frequency"] * dt:
            ...
            impulse = np.random.uniform(-self.physical_params["disturbance_magnitude"], 
        ...
        friction = b_f * x_dot * abs(x_dot)  # Friction proportional to speed^2, opposing motion
        ...
        x_acc = (motor_torque - m_b * h * theta_acc * np.cos(theta) - friction - rolling_resistance) / M
        ...
        # Update state
        self.state = np.array([x_new, x_dot_new, theta_new, theta_dot_new, left_wheel_new, right_wheel_new])
        ...
        wheel_rpm = (left_wheel_speed + right_wheel_speed) * 30 / np.pi  # Convert rad/s to RPM
        wheel_rpm += np.random.normal(0, sensor_noise * abs(wheel_rpm)) if wheel_rpm != 0 else 0
        ...
        speed = x_dot_new + np.random.normal(0, sensor_noise * abs(x_dot_new)) if x_dot_new != 0 else 0
    ...
    def forward(self, target_speed=0, turn_bias=0):
        ...
        if abs(current_angle) > tilt_safety_threshold:
            ...
            power_reduction = tilt_factor * abs(target_speed) * 0.8  # 80% reduction at max
    ...
    try:
        while time.time() - start_time < duration:
            ...
            if use_controller:
                ...
                angle, left, right, speed, accel = robot.forward(target_speed=target_speed)
            else:
            ...
            # Print current state
            print(f"{elapsed:5.1f}s | {angle:+8.2f}° | {speed:+8.2f}m/s | {accel:+8.2f} | {left:+8.1f} | {right:+8.1f}")
...
def create_simulation_with_measured_params():
    ...
    try:
        ...
        body_height = input("Height of center of mass from wheel axis in meters (default 0.15): ")
        if body_height:
...
def measure_parameter_guide():
    ...
    print("   - Method 2: Hang the robot body from two different points and find the intersection")
    ...
    print("   - Place the robot on a slight incline and find the angle where it starts rolling")
    ...
    if choice == "1":
        ...
        try:
            while time.time() - start_time < duration:
                ...
                angle, left, right, speed, accel = robot.forward(target_speed=target_speed)
                print(f"Angle: {angle:+6.2f}° | Speed: {speed:+5.2f}m/s | Motors: {left:+5.1f}, {right:+5.1f}")
                time.sleep(0.1)
        except KeyboardInterrupt:
            pass
        finally:
